...
What I'm still unsure about is that, in the functional-php implementation, they passed an identity function (x => x) as the initialValue for the reduce function. It may have something to do with how array_reduce works in PHP - in JavaScript, initialValue defaults to the first item in the array and skips the first item when setting current in the callback function, so it doesn't seem to cause any problems leaving this out. But maybe this is not the case for PHP's array_reduce?
